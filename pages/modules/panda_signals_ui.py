...
def signals_block():
    ...
    def _collect_traces_for_positions(fusion_df: pd.DataFrame,
        ...
        fx = fusion_df.copy()
        fx[pos_col_fusion] = pd.to_numeric(fx[pos_col_fusion].astype(str).str.strip(), errors="coerce")
        fx = fx.dropna(subset=[pos_col_fusion])

        meta_cols = list(fx.columns)  # inclut Eps_1, Eps_2, Ind_1, Ind_2 si présents
        ...
        for upl in (ant_files or []):
            ...
            for sheet in sheets_to_read:
                ...
                for ridx, (p_int, sig_vals) in enumerate(zip(df["__P__"], sig_df.values)):
                    if p_int not in fusion_meta_by_pos.index:
                        continue
                    meta = fusion_meta_by_pos.loc[p_int]
                    meta_list = [meta[c] if c in meta.index else np.nan for c in meta_cols]
        ...
        out_df = out_df.drop(columns=["sheet_name", "trace_index"], errors="ignore")

        # Mettre la colonne position en première
        if pos_col_fusion in out_df.columns:
            cols = [pos_col_fusion] + [c for c in out_df.columns if c != pos_col_fusion]
            out_df = out_df[cols]

        return out_df

    # === A) Charger Fusion
    st.subheader("A. Charger le fichier **Fusion Panda+GPR** (incluant Eps/Indicateurs)")
    ...
    ant_files = st.file_uploader(
        "Fichiers ANT_..._start_end.xlsx (plusieurs)",
        type=["xlsx", "xls"], accept_multiple_files=True, key="upl_ant_files"
    )
    if ant_files:

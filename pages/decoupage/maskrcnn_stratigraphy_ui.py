...
def stratigraphy_block():
    ...
    def _ensure_raw_loaded(category: str):
        ...
        if not scan:
            return None
        if category == "ASC":
            if st.session_state.df_asc_raw is None:
            ...
            if st.session_state.df_desc_raw is None:
    ...
    def _strato_block(category: str):
        ...
        with cols[1]:
            st.number_input("Longueur image (ns)", min_value=0.0, step=1.0,
                            key=t_key, value=st.session_state[t_key])
        with cols[2]:
            st.number_input("Hauteur réelle (px)", min_value=0.0, step=1.0,
                            key=h_key, value=st.session_state[h_key])
        with cols[3]:
            st.radio("Style",
        ...
        selected_interfaces = st.multiselect("Choisis les interfaces à afficher :", all_interfaces, default=all_interfaces)

        # --- bornes min / max (toujours affichées)
        ...
        if xmin_key not in st.session_state: st.session_state[xmin_key] = xmin_all
        if xmax_key not in st.session_state: st.session_state[xmax_key] = xmax_all

        b1, b2 = st.columns(2)
        with b1:
            st.number_input("Borne min x_cm",
                            min_value=float(xmin_all), max_value=float(xmax_all),
                            step=10.0, key=xmin_key, value=st.session_state[xmin_key])
        with b2:
            st.number_input("Borne max x_cm",
                            min_value=float(xmin_all), max_value=float(xmax_all),
                            step=10.0, key=xmax_key, value=st.session_state[xmax_key])
        ...
        trend_win = st.session_state.get(trend_key, 5)

        xmin_sel  = _safe_float(st.session_state.get(xmin_key), xmin_all) or xmin_all
        xmax_sel  = _safe_float(st.session_state.get(xmax_key), xmax_all) or xmax_all
